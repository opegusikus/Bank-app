...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label1->Text = L"Оберіть картку";
			// 
			// label2
			// 
			...
			this->radioButton1->Text = L"Відкрити депозит";
			...
			this->radioButton2->Text = L"Подивитися";
			...
			this->label3->Text = L"Кількість місяців";
			// 
			// textBox2
			// 
		...
		try {
			if (radioButton1->Checked)
			{
				...
				if (oldDepo == nullptr) {
					if (sum <= loginAccount->Cards[index]->Balance)
					{
						...
						depRepo->Create(depo);
						MessageBox::Show("Депозит відкрито!");
						this->Close();
					}
					else
					{
						MessageBox::Show("Недостатньо коштів!", "Увага!", MessageBoxButtons::OK, MessageBoxIcon::Error);
					}
				}
				else
				{
					MessageBox::Show("У вас вже є відкритий депозит", "Увага!", MessageBoxButtons::OK, MessageBoxIcon::Error);
				}
			}
			else if (radioButton2->Checked) 
			{
				...
				if (oldDepo != nullptr) 
				{
					...
					label1->Text = "Вкладена сума: " + oldDepo->Sum.ToString() + " грн";
					label2->Text = "Кількість місяців: " + oldDepo->Month;
					label3->Text = "Процентна ставка: " + oldDepo->InterestRate.ToString()+" %";
				}
				else
				{
					MessageBox::Show("У вас немає відкритого депозиту", "Увага!", MessageBoxButtons::OK, MessageBoxIcon::Error);
				}
			}
